...
class News extends Admin_Controller
{
    ...
    public function ajax_edit($id)
    {
        ...
        if (!empty($data['data'][0]->category_id)){
            $oneCate = $categoryPage->getById($data['data'][0]->category_id,'',$this->session->admin_lang);
    ...
    private function _validate()
    {
        if($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            if (!empty($this->config->item('cms_language'))) foreach ($this->config->item('cms_language') as $lang_code => $lang_name) {
                    ...
                    $this->form_validation->set_rules('category_id', ' loại tin tức ' , 'required');
            }
    ...
    public function ajax_update_filter(){
        $this->checkRequestGetAjax();
        $keyword = toSlug(toNormal($this->input->get("q")));
        $params['keyword'] = $keyword;
        $data    = $this->_data->filter_internal($params);
